...
export async function verifyAzureCLI(context: IActionContext): Promise<void> {
    try {
        ...
        ext.outputChannel.appendLog(stdout, { resourceName: "az --version stdout" });
        ext.outputChannel.appendLog(stderr, { resourceName: "az --version stderr" });
    } catch (error) {
        ext.outputChannel.appendLog((error as ExecException).message, { resourceName: "az --version error" });
    ...
    try {
        ...
        ext.outputChannel.appendLog(stdout, { resourceName: "az account show" });
        ext.outputChannel.appendLog(stderr, { resourceName: "az account show" });
    } catch (error) {
        ext.outputChannel.appendLog((error as ExecException).message, { resourceName: "az account show error" });
        void context.ui.showWarningMessage(
