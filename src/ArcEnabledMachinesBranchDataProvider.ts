...
export class ArcEnabledMachinesBranchDataProvider implements AzureResourceBranchDataProvider<ArcEnabledMachinesResourceModel> {
    getChildren(_element: ArcEnabledMachinesResourceModel): ArcEnabledMachinesResourceModel[] {
        // Arc-enabled machines have no children.
        return [];
    }

    async getResourceItem(element: AzureResource): Promise<ArcEnabledMachinesResourceModel> {
        ...
        const resourceItem = await callWithTelemetryAndErrorHandling(
            ...
            async (context: IActionContext) => {
                ...
                const arcEnabledMachine = await ArcEnabledMachineItem.Get(
                    ...
                    element.name);
                return new ArcEnabledMachineItem(element.subscription, element, arcEnabledMachine);
            });
